...
#else
  #define CASADI_PREFIX(ID) cineq3_max_func_ ## ID
#endif
...
/* cineq3_max_func:(i0[180],i1)->(o0[30]) */
...
#ifdef MATLAB_MEX_FILE
void mex_cineq3_max_func(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  casadi_real w[391];
  casadi_int *iw = 0;
  const casadi_real* arg[2] = {0};
  casadi_real* res[1] = {0};
  if (argc>2) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"cineq3_max_func\" failed. Too many input arguments (%d, max 2)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"cineq3_max_func\" failed. Too many output arguments (%d, max 1)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+211);
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+180, casadi_s1, w+211);
  --resc;
  res[0] = w+181;
  i = cineq3_max_func(arg, res, iw, w+211, 0);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"cineq3_max_func\" failed.");
  if (res[0]) resv[0] = casadi_to_mex(casadi_s2, res[0]);
}
#endif

casadi_int main_cineq3_max_func(casadi_int argc, char* argv[]) {
  casadi_int j;
  casadi_real* a;
  const casadi_real* r;
  casadi_int flag;
  ...
  flag = cineq3_max_func(arg, res, iw, w+211, 0);
  if (flag) return flag;
  r = w+181;
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_cineq3_max_func(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "cineq3_max_func")==0) {
    mex_cineq3_max_func(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'cineq3_max_func'");
}
#endif
int main(int argc, char* argv[]) {
  if (argc<2) {
    /* name error */
  } else if (strcmp(argv[1], "cineq3_max_func")==0) {
    return main_cineq3_max_func(argc-2, argv+2);
  }
  fprintf(stderr, "First input should be a command string. Possible values: 'cineq3_max_func'\nNote: you may use function.generate_input to create a command string.\n");
  return 1;
}
#ifdef __cplusplus
} /* extern "C" */
