function [COM_next, dCOM_next, ddCOM_next, p_err_sum_x_next, p_err_sum_y_next] = previewControl(t_step, step_phase, p_total, T_step_total, Gi, Gx, Gp, A, B, C, COM, dCOM, ddCOM, p_err_sum_x, p_err_sum_y)

    COM_x_dx_ddx = [COM(1); dCOM(1); ddCOM(1)];
    COM_y_dy_ddy = [COM(2); dCOM(2); ddCOM(2)];

    p_window = zmpTrajWindow(t_step, step_phase, p_total, T_step_total);
    ...
    for k2 = 1:PARA.NL
        temp_x2 = temp_x2 + Gp(k2)*p_window(1, k2+1);
        temp_y2 = temp_y2 + Gp(k2)*p_window(2, k2+1);
    end
